...
from real.envs import locomotion_gym_config
from real.robots import (a1_robot_velocity_estimator, laikago_constants,
                         laikago_motor, minitaur, robot_config)
...
URDF_FILENAME = os.path.join(parentdir, "real/utilities/a1/a1.urdf")
...
class A1(minitaur.Minitaur):
    ...
    """
        if motor_control_mode == robot_config.MotorControlMode.TORQUE:
            return np.clip(motor_commands, -1 * self._motor_torque_limits,
                           self._motor_torque_limits)
        ...
        if motor_control_mode == robot_config.MotorControlMode.HYBRID:
            ...
            clipped_torques = np.clip(torques, -1 * self._motor_torque_limits,
                                      self._motor_torque_limits)
            motor_commands[np.array(range(NUM_MOTORS)) * 5 +
                           4] = clipped_torques
