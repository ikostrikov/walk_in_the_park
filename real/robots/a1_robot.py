...
from real.robots import a1, laikago_pose_utils, minitaur, robot_config
...
class A1Robot(a1.A1):
    ...
    def _CheckMotorTemperatures(self):
        if any(self._motor_temperatures > MOTOR_WARN_TEMP_C):
            ...
            for name, temp in zip(MOTOR_NAMES,
                                  self._motor_temperatures.astype(int)):
                print(f"{name}: {temp} C")

    def _UpdatePosition(self):
        now = time.time()
        self._base_position += self.GetBaseVelocity() * (
            now - self._last_position_update_time)
    ...
    """
        ...
        self._hold_process = multiprocessing.Process(
            target=self._HoldPose,
            args=(self.GetMotorAngles(), self._child_pipe))
    ...
    def _StandupReset(self, default_motor_angles, reset_time):
        ...
        for t in np.arange(0, standup_time,
