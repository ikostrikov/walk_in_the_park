...
class FC:
    """Represents a fully-connected layer in a network. (code from: https://github.com/JannerM/mbpo/blob/master/mbpo/models/fc.py)
    """
    _activations = {
        None: tf.identity,
        "ReLU": tf.nn.relu,
    ...
    def set_model_vars(self, variables):
        ops = [
            getattr(self, attr).assign(var) for attr, var in variables.items()
        ]
        return ops
    ...
    def compute_output_tensor(self, input_tensor):
        ...
        if len(input_tensor.shape) == 2:
            raw_output = tf.einsum("ij,ajk->aik", input_tensor,
                                   self.weights) + self.biases
        elif len(input_tensor.shape
                 ) == 3 and input_tensor.shape[0].value == self.ensemble_size:
