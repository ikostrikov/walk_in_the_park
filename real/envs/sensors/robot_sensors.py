...
from real.envs.sensors import sensor
...
class MotorAngleSensor(sensor.BoxSpaceSensor):
    ...
    """
        ...
        if observe_sine_cosine:
            ...
            super(MotorAngleSensor, self).__init__(name=name,
...
class MinitaurLegPoseSensor(sensor.BoxSpaceSensor):
    ...
    def _get_observation(self) -> _ARRAY:
        motor_angles = (self._robot.GetMotorAngles() if self._noisy_reading
...
class BaseDisplacementSensor(sensor.BoxSpaceSensor):
    ...
    def _get_observation(self) -> _ARRAY:
        ...
        if self._convert_to_local_frame:
            dx_local = np.cos(self._last_yaw) * dx + np.sin(
                self._last_yaw) * dy
            dy_local = -np.sin(self._last_yaw) * dx + np.cos(
                self._last_yaw) * dy
...
class IMUSensor(sensor.BoxSpaceSensor):
    ...
    """
        ...
        super(IMUSensor, self).__init__(name=name,
                                        shape=(self._num_channels, ),
                                        lower_bound=lower_bound,
                                        upper_bound=upper_bound,
                                        dtype=dtype)
