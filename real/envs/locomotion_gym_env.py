...
from real.envs.sensors import sensor, space_utils
...
class LocomotionGymEnv(gym.Env):
    ...
    def _build_action_space(self):
        ...
        if motor_mode == robot_config.MotorControlMode.HYBRID:
            ...
            torque_limits = np.array([100] *
                                     len(self._robot_class.ACTION_CONFIG))
    ...
    """
        ...
        if self._hard_reset:
            ...
            self._robot = self._robot_class(
                ...
                reset_at_current_position=self._gym_config.
                simulation_parameters.reset_at_current_position,
                motor_torque_limits=self._gym_config.simulation_parameters.
                torque_limits,
                enable_clip_motor_commands=self._gym_config.
                simulation_parameters.enable_clip_motor_commands,
                enable_action_filter=self._gym_config.simulation_parameters.
                enable_action_filter,
                enable_action_interpolation=self._gym_config.
                simulation_parameters.enable_action_interpolation,
    ...
    """
        ...
        if self._is_render:
            ...
            if hasattr(self._task, '_ref_model'):
                self._pybullet_client.changeVisualShape(self._task._ref_model,
                                                        -1,
                                                        rgbaColor=ref_col)
                for l in range(
                        self._pybullet_client.getNumJoints(
                            self._task._ref_model)):
    ...
    """
        ...
        self._pybullet_client.setPhysicsEngineParameter(
            numSolverIterations=int(
                np.round(self._num_bullet_solver_iterations)))
        self._pybullet_client.setTimeStep(self._sim_time_step)
    ...
    def set_task(self, new_task):
        ...
        if hasattr(self._task,
                   "_ref_model") and self._task._ref_model is not None:
