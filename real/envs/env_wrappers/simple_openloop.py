...
from real.robots import laikago_pose_utils, minitaur_pose_utils
...
class A1PoseOffsetGenerator(object):
    """A trajectory generator that return constant motor angles."""
    def __init__(self, action_limit=[0.2, 0.4, 0.4]):
        """Initializes the controller.
    Args:
      action_limit: a tuple of [limit_abduction, limit_hip, limit_knee]
    """
        self._pose = np.asarray([0, 0.9, -1.8] * 4)
        action_high = np.asarray(action_limit * 4)
        self.action_space = spaces.Box(-action_high,
                                       action_high,
                                       dtype=np.float32)
